...
GavelAnnotatorSchema.methods.canVote = async function() {
    if (!this.next) {
        return Promise.reject(new ForbiddenError('Cannot submit votes without a project assigned'));
    }

    if (!this.active) {
        return Promise.reject(new ForbiddenError('Cannot submit votes while disabled'));
    }

    if (!this.onboarded) {
        return Promise.reject(new ForbiddenError('Cannot submit votes before being onboarded'));
    }

    const event = await Event.findById(this.event);
    if (!event) {
        return Promise.reject(new ForbiddenError('Annotator belongs to no event'));
    }

    if (!EventHelpers.isVotingOpen(event, moment)) {
        return Promise.reject(new ForbiddenError('Cannot submit votes while voting is not open'));
    }
...
GavelAnnotatorSchema.methods.getPreferredProjects = async function() {
    ...
    const [availableProjects, activeAnnotators] = await Promise.all([availableProjectsQuery, activeAnnotatorsQuery]);
