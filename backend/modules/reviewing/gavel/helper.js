    ...
    return Promise.map(
        ...
        slug => {
            ...
            return Promise.map(
                sorted,
                (project, index) => {
                    return Achievement.createTrackPlacementAchievements(event, project.project, slug, index + 1);
                },
                {
                    concurrency: 5
                }
...
GavelHelper.generateOverallPlacementAchievements = async event => {
    ...
    if (event.tracksEnabled) {
        ...
        return Promise.map(
            sorted,
            (project, index) => {
                return Achievement.createOverallPlacementAchievements(event, project.project, index + 1);
            },
            {
                concurrency: 5
            }
