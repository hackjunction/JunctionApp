...
function getRoles(access_token) {
    return axios
        .get(`${global.gConfig.AUTH0_AUTHORIZATION_EXTENSION_URL}/roles`, config(access_token))
...
function removeRole(access_token, userId, roleId) {
    return axios
        .delete(`${global.gConfig.AUTH0_AUTHORIZATION_EXTENSION_URL}/users/${userId}/roles`, {
            ...config(access_token),
            data: [roleId]
        })
        .then(res => res.data);
...
controller.grantAssistantOrganiser = async userId => {
    ...
    const role = await getRoleByName(access_token, AuthConstants.Roles.ASSISTANT_ORGANISER);
