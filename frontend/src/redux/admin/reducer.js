    ...
    switch (action.type) {
        case ActionTypes.LOAD_USERS: {
            return handle(state, action, {
                start: prevState => ({ ...prevState, usersLoading: true, usersError: false }),
                finish: prevState => ({ ...prevState, usersLoading: false }),
                failure: prevState => ({ ...prevState, usersError: true }),
                success: prevState => ({ ...prevState, users: action.payload, usersUpdated: Date.now() })
            });
        }
        case ActionTypes.LOAD_ROLES: {
            return handle(state, action, {
                start: prevState => ({ ...prevState, rolesLoading: true, rolesError: false }),
                finish: prevState => ({ ...prevState, rolesLoading: false }),
                failure: prevState => ({ ...prevState, rolesError: true }),
                success: prevState => ({ ...prevState, roles: action.payload, rolesUpdated: Date.now() })
            });
        }
