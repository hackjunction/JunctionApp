...
const TravelGrantsPage = ({ items, event, idToken, enqueueSnackbar }) => {
    ...
    const notifyRejected = useCallback(async () => {
        ...
        try {
            const {
                count,
            } = await RegistrationsService.adminNotifyRejectedTravelGrants(
                idToken,
                event.slug
            )
            enqueueSnackbar(`Success! Notification set to ${count} people`, {
                variant: 'success',
            })
        } catch (e) {
    ...
    const notifyAccepted = useCallback(async () => {
        ...
        try {
            const {
                count,
            } = await RegistrationsService.adminNotifyAcceptedTravelGrants(
                idToken,
                event.slug
            )
            enqueueSnackbar(`Success! Notification set to ${count} people`, {
    ...
    return (
        <>
            <PageHeader
                ...
                subheading="Manage travel grants"
            />
