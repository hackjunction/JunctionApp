        ...
        switch (status) {
            ...
            case RegistrationStatuses.asObject.softRejected.id: {
                if (isRegistrationOpen) {
                    return (
                        <Button
                            onClick={() => editRegistration(event.slug)}
                            color="theme_white"
                            variant="contained"
                        >
                            Edit registration
            ...
            case RegistrationStatuses.asObject.confirmed.id: {
                return (
                    ...
                    >
                        ...
                        <Box ml={1} mt={1}>
                            <Button
                                onClick={() => openEventId(event.slug)}
                                color="theme_white"
                                variant="contained"
                            >
                                Your event ID
...
const mapDispatch = dispatch => ({
    confirmRegistration: slug =>
        dispatch(DashboardActions.confirmRegistration(slug)),
    cancelRegistration: slug =>
        dispatch(DashboardActions.cancelRegistration(slug)),
    editRegistration: slug => dispatch(push(`/events/${slug}/register`)),
    openEventId: slug => dispatch(push(`/dashboard/${slug}/event-id`)),
})

export default withSnackbar(
    connect(mapState, mapDispatch)(RegistrationStatusBlock)
)
