...
export default () => {
    ...
    const registrationsLoading = useSelector(OrganiserSelectors.registrationsLoading);
    const { slug } = event;
    ...
    const handleSelfAssign = useCallback(() => {
        ...
        RegistrationsService.assignRandomRegistrations(idToken, slug)
            .then(data => {
                if (data === 0) {
                    dispatch(SnackbarActions.show('No available registrations left to assign!'));
                } else {
                    dispatch(SnackbarActions.success(`Assigned ${data} registrations to you`));
                }
            })
            .catch(() => {
