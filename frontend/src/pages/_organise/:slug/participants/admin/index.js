...
export default () => {
    ...
    const handleBulkAccept = () => {
        return RegistrationsService.bulkAcceptRegistrationsForEvent(idToken, event.slug)
            .then(data => {
                dispatch(SnackbarActions.success('Success! All soft accepted registrations have been accepted.'));
            })
            .catch(err => {
                dispatch(
                    SnackbarActions.error(`Something went wrong... Are you sure you're connected to the internet ?`)
                );
            })
            .finally(() => {
    ...
    const handleBulkReject = () => {
        return RegistrationsService.bulkRejectRegistrationsForEvent(idToken, event.slug)
            .then(data => {
                dispatch(SnackbarActions.success(`Success! All soft rejected registrations have been rejected.`));
            })
            .catch(err => {
                dispatch(
                    SnackbarActions.error(`Something went wrong... Are you sure you're connected to the internet?`)
