...
export default () => {
    ...
    useEffect(() => {
        dispatch(OrganiserActions.updateOrganisersForEvent(event.owner, event.organisers)).catch(() => {
            dispatch(
                SnackbarActions.error('Oops, something went wrong... Unable to load organisers. Please try again.')
            );
        });
    ...
    const handleOrganiserRemoved = useCallback(
        userId => {
            ...
            dispatch(OrganiserActions.removeOrganiserFromEvent(slug, userId))
                ...
                .catch(() => {
                    dispatch(SnackbarActions.error('Oops, something went wrong... Unable to remove organiser'));
                })
                .finally(() => {
                    setLoading(false);
                });
    ...
    const handleOrganiserAdded = useCallback(
        userId => {
            ...
            dispatch(OrganiserActions.addOrganiserToEvent(slug, userId))
                ...
                .catch(err => {
                    dispatch(SnackbarActions.error('Oops, something went wrong... Please try again.'));
                })
                .finally(() => {
                    setLoading(false);
                });
