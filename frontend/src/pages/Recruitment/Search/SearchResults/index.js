  ...
  const renderResults = () => {
    if (searchResults.length === 0) {
      if (filters.textSearch) {
        ...
        return (
          <Empty isEmpty emptyText="No results with the selected filters." />
        );
      }
    }
    return (
      <Grid direction="row" alignItems="stretch" container spacing={2}>
        {searchResults.map(item => (
          <Grid
            ...
            lg={3}
          >
            <ResultCard data={item} onClick={e => openDetail(item.userId)} />
          </Grid>
        ))}
      </Grid>
    );
...
const mapDispatch = (dispatch, ownProps) => ({
  updateSearchResults: ownProps.items
    ? () => {}
  ...
  openDetail: path => dispatch(push(`/recruitment/${path}`))
});
