        ...
        return (
            <Grid item mb={1}>
                ...
                {areas.map(area => {
                    return (
                        <Typography key={area}>
                            <CheckCircleRoundedIcon fontSize="inherit" className={styles.listItem} />
                            {area}
                        </Typography>
                    );
                })}
...
const RecruitmentProfileInfo = React.memo(({ participant, sendMessage, messages }) => {
    const { themesOfInterest, industriesOfInterest, skills, education, roles } = participant;
    const { firstName } = participant.profile;

    const [messageVal, changeMessageValue] = useState('');

    return (
        <Grid container direction="row" justify="center" justifyContent="center" xs={12}>
            <Grid container item direction="column" wrap="nowrap" justifyContent="center" xs={12} sm={4} spacing={4}>
            ...
            <Grid container item direction="column" xs={12} wrap="nowrap" sm={8}>
                ...
                <Grid item>
                    ...
                    <Button
                        variant="contained"
                        color="secondary"
                        onClick={() => (sendMessage(messageVal, participant.userId), changeMessageValue(''))}
                        startIcon={<EmailIcon />}
                    >
                        Send
                    </Button>
...
const mapState = (state, ownProps) => {
    ...
    const messages = actionHistory.filter(
        action => action.type === 'message' && action.user === ownProps.participant.userId
    );
    return {
        messages
    };
